<?php
 ...
 $grafico->SetTitle("Fechamentos 2026");
...
for ($i = 1; $i <= 12; $i++) {
    ...
    $stmt = $con->prepare("SELECT DISTINCT COUNT(id_serv) FROM ordem_serv WHERE YEAR(`data_os_instalacao`)= '$ano_2026' AND MONTH(`data_os_instalacao`) = '$i'");
    $stmt->execute();
    $stmt->store_result();
    $num_of_rows = $stmt->num_rows;
    if ($stmt->num_rows > 0) {
