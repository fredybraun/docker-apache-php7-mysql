...
<h3>Marni</h3>
	...
	<?php
				if ($semana_get) {
				    $stmt = $con->prepare("SELECT id_os, id_cli, id_status, id_serv, tempo_os, agendamento_os FROM ordem_serv WHERE semana_os='$semana_get' AND id_func = '1' ORDER BY id_status");
				    $stmt->execute();
				    $stmt->store_result();
				    $num_of_rows = $stmt->num_rows;
				    if ($stmt->num_rows > 0) {
				        ...
				        while ($stmt->fetch()) {
                     		echo '<div class="row bg-light w-75"><div class="col-1"><a class="text-dark" href="insert_os.php?editar='.$id_os.'">'.$id_os.'</a></div>';
                     		switch ($id_status){
                     			case 1 :
                     				$color  = "bg-warning";
                     				$text = "text-dark";
	                            ...
	                            if ($stmt2->num_rows > 0) {
	                                ...
	                                while ($stmt2->fetch()) {
											 ...
											 if(!($agendamento == NULL) && !($agendamento == '1970-01-01') && ($agendamento == $data_agendamento)){
												...
												echo '<div class="col-5 '.$color.'"><a class="'.$text.'" href="dados_cliente.php?id_cliente='.$id_cliente.'">'.$nome_cliente.'</a></div>';
											}
...
<br><h3>Daian</h3>
	...
	<?php
				if ($semana_get) {
				    $stmt = $con->prepare("SELECT id_os, id_cli, id_status, id_serv, tempo_os, agendamento_os FROM ordem_serv WHERE semana_os='$semana_get' AND id_func = '3' ORDER BY id_status");
				    $stmt->execute();
				    $stmt->store_result();
				    $num_of_rows = $stmt->num_rows;
				    if ($stmt->num_rows > 0) {
...
<br><h3>Patrick</h3>
	...
	<?php
				if ($semana_get) {
				    $stmt = $con->prepare("SELECT id_os, id_cli, id_status, id_serv, tempo_os, agendamento_os FROM ordem_serv WHERE semana_os='$semana_get' AND id_func = '31' ORDER BY id_status");
				    $stmt->execute();
				    $stmt->store_result();
				    $num_of_rows = $stmt->num_rows;
				    if ($stmt->num_rows > 0) {
