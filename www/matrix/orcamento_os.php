<?php
session_start();
$_SESSION["onde_estou"] = 'orcamento_os';
require ('./topo2.php');
require ('./funcoes.php');
?>
...
<div class="container-fluid mt-2 col-md-12">
	<h3>Orçamento</h3>
					 <table class="table table-sm">
						...
						<?php
							if ($stmt = $con->prepare('SELECT id_os, id_cli, id_func, id_serv, id_status, data_os, obs_os FROM ordem_serv WHERE id_status = "6" ORDER BY id_os DESC')) {
								$stmt->execute();
								$stmt->store_result();
								$num_of_rows = $stmt->num_rows;
								if ($stmt->num_rows > 0) {
									$stmt->bind_result($id_os, $id_cli, $id_func, $id_serv, $id_status, $data_os, $obs_os);
									while ($stmt->fetch()) {
